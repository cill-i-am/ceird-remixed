...
description: Use when working on the TanStack Start app routing layer in this repository, especially `apps/app` routes, file-based route conventions, root route setup, route loaders, `beforeLoad`, router context, TanStack Query SSR preloading, search/path params, navigation, code splitting, server functions, server routes, middleware, SSR/hydration, or deployment/runtime behavior. This skill routes agents to the smallest official TanStack skill shipped in node_modules, then applies this repo's app-boundary rules without duplicating upstream docs.
---

# TanStack Routing

This is a thin repo-local routing layer over the official TanStack skills bundled with the installed packages. Start with `apps/AGENTS.md`, then use the routing table to pick the relevant TanStack reference for the task.
...
- Keep TanStack Start routing, loaders, server functions, and app-shell wiring inside `apps/app`.
