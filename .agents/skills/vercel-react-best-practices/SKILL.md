...
This skill adapts the Vercel React performance rule catalog to this repository's stack: Vite, TanStack Start, TanStack Router, TanStack Query, Cloudflare deployment, Effect, and pnpm.
...
- Keep cross-request caches explicit, bounded, and non-user-specific unless keyed by a parsed principal. Prefer Effect cache utilities for server-side source caching in this repo.
...
The compiled guide in `AGENTS.md` is a sanitized index for this repo. The individual rule files remain the source of detailed examples.
