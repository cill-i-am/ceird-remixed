...
Before doing any other Effect-related work, verify that this repo can resolve Effect source through opensrc:

```sh
pnpm exec opensrc path --cwd . effect
```

If the command fails, read `./references/source-lookup.md` and run the documented setup. Do not require or create `./.repos/effect` for this repository.
