...
Use opensrc for Effect source code. This repository intentionally does not use a project-local `./.repos/effect` checkout.
...
The returned path may live under `Effect-TS/effect-smol/.../packages/effect` because that is the npm package's source repository metadata. Do not fetch `Effect-TS/effect-smol` separately for this repo; treat the `effect` package path as the single source reference.
...
- Do not create, require, or commit `./.repos/effect` in this repository.
