...
This repository uses opensrc for Effect source lookup. Do not require a project-local `./.repos/effect` checkout.
...
If `effect` is installed from the npm beta tag, keep using `pnpm exec opensrc path --cwd . effect`; opensrc resolves the version from the workspace lockfile. The returned package path is the single Effect source reference for this repo.
...
- Do not commit fetched Effect source into this application repo.
